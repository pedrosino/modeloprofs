...
desvios = LpVariable.matrix("zd", m_unidades[:n_unidades, 0], cat="Integer")
if(modulo):
    desviosabs = LpVariable.matrix("zda", m_unidades[:n_unidades, 0], cat="Integer")
medias = LpVariable.matrix("zm", m_unidades[:n_unidades, 0], cat="Integer")
media_geral = LpVariable("zmg", cat="Integer")
...
if(modo == 'num'):
    ...
    model += lpSum(saida*matriz_tempon)
elif(modo == 'ch'):
    if(modulo):
        model += lpSum(desviosabs[:n_unidades])
    else:    
        model += lpSum(desvios[:n_unidades])
...
if(modo == 'ch'):
    ...
    model += media_geral == m*lpSum(saida[:n_unidades]) + ch_min*fator + ch_max*fator, "Media geral"
    for u in range(n_unidades):
        ...
        # calculo da media
        model += medias[u] == m*lpSum(saida[u]) + ch_min*fator + ch_max*fator, f"{m_unidades[u][0]}_ch_media"
        # desvio
        if(modulo):
            ...
            model += desvios[u] >= media_geral - medias[u], f"{m_unidades[u][0]}_low"
...
# Resolver o modelo
#status = model.solve()
status = model.solve(PULP_CBC_CMD(msg=1, maxSeconds=time_limit))
...
print(f"--------+---------------------------------+-------+--------+-------+------------+")
for u in range(n_unidades):
    print(f"{m_unidades[u][0]}   | " + " ".join([f"{qtdes[u][p]:3d}" for p in range(n_perfis)]) + f" |  {np.sum(qtdes[u]):4d} | {np.sum(qtdes[u]*matriz_peq):6.2f} |  {np.sum(qtdes[u]*matriz_tempon)*-1:4d} |    {(np.sum(qtdes[u]*matriz_tempon)*-1)/np.sum(qtdes[u]):7.3f} |")
print(f"--------+---------------------------------+-------+--------+-------+------------+")
