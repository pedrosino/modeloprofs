...
numero_max = round(np.sum(m_unidades[:n_unidades], axis=0)[1] / ch_min)
...
filename = f"CBC completo {datetime.now().strftime('%H-%M-%S')}.txt"
...
for modo in modos:
    ...
    if(modo == 'num'):
        ...
        modelos[modo] += lpSum(saida*matriz_tempo)
    elif(modo == 'ch'):
    ...
    # Resolver o modelo
    #status = model.solve()
    status = modelos[modo].solve(PULP_CBC_CMD(msg=0, timeLimit=time_limit))
    ...
    df.to_excel(f'CBC{modo}{"q" if quarenta else ""}-prel.xlsx', sheet_name='Resultados', index=False)
...
modelo += pontuacoes[2] == (lpSum(saida*matriz_tempo) - piores['tempo'])/(melhores['tempo'] - piores['tempo'])
...
# Resolver o modelo
#status = model.solve()
status = modelo.solve(PULP_CBC_CMD(msg=1, timeLimit=time_limit))
#status = model.solve(GLPK(msg=True))
...
print(f"--------+---------------------------------+-------+--------+-------+------------+")
for u in range(n_unidades):
    print(f"{m_unidades[u][0]:5s}   | " + " ".join([f"{qtdes[u][p]:3d}" for p in range(n_perfis)]) + f" |  {np.sum(qtdes[u]):4d} | {np.sum(qtdes[u]*matriz_peq):6.2f} |  {np.sum(qtdes[u]*matriz_tempo):4d} |    {(np.sum(qtdes[u]*matriz_tempo))/np.sum(qtdes[u]):7.3f} |")
print(f"--------+---------------------------------+-------+--------+-------+------------+")
print(f"Total   | " + " ".join([f"{np.sum(qtdes, axis=0)[p]:3d}" for p in range(n_perfis)]) + f" |  {np.sum(qtdes):4d} | {np.sum(qtdes*matriz_peq):6.2f} |  {np.sum(qtdes*matriz_tempo):4d} |    {(np.sum(qtdes*matriz_tempo))/np.sum(qtdes):7.3f} |")
print(f"--------+---------------------------------+-------+--------+-------+------------+")
print("")
...
for u in range(n_unidades):
    ...
    + " ".join([f"{np.sum(qtdes[u]*m_perfis[p]):4d} (+{(np.sum(qtdes[u]*m_perfis[p]) - m_unidades[u][p+1]):3d}) |" for p in range(n_restricoes)])
...
print(f"--------+-------------+-------------+-------------+-------------+-------------+----------+----------+----------+")
print(f"Total   | " + " ".join([f"{np.sum(qtdes*m_perfis[p]):4d} (+{np.sum(qtdes*m_perfis[p]) - int(np.sum(m_unidades, axis=0)[p+1]):3d}) |" for p in range(n_restricoes)])
