...
if(modulo):
    desviosabs = LpVariable.matrix("zda", m_unidades[:n_unidades, 0], cat="Continuous")
...
if(modo == 'ch'):
    ...
    for u in range(n_unidades):
        ...
        # calculo da media
        model += medias[u] == m*lpSum(saida[u]) + ch_min + ch_max, f"{m_unidades[u][0]}_ch_media"
        # desvio
        if(modulo):
...
filename = f"CBC2{modo}_n{n_unidades}-Ma{maxima}-Mi{minima}-Peq{peq}-Tot{n_min_total if min_total else min_total}-{n_max_total if max_total else max_total}-Dif{dif_tam if dif else dif}-40h{quarenta}-20h{vinte}--{datetime.now().strftime('%H-%M-%S')}.txt"
...
df.to_excel(f'CBC2{modo}-Peq{peq}-test.xlsx', sheet_name='Resultados', index=False)
