...
# Decision variables
deviation = [LpVariable(f"deviation_{u}", cat="Continuous") for u in range(n_depts)]  # Define deviation variables
avg_classes = [LpVariable(f"avg_classes_{u}", lowBound=0) for u in range(n_depts)]  # Average classes per teacher variables
...
model += lpSum(deviation[u] for u in range(n_depts))  # Minimize deviation
...
# Constraints to calculate average classes per teacher and update deviation variables
for u in range(n_depts):
    model += lpSum(output[u]) == dept_matrix[u][1]  # Total classes in department u

    # Calculate the average classes per teacher
    model += avg_classes[u] == lpSum(output[u]) / lpSum(output[u][p] for p in range(n_profiles))

    # Calculate the deviation from the global average
    model += deviation[u] >= avg_classes[u] - (total_classes / lpSum(output * teacher_matrix))
    model += deviation[u] >= (total_classes / lpSum(output * teacher_matrix)) - avg_classes[u]

# Constraint to calculate total classes taught
model += total_classes == lpSum(output)
