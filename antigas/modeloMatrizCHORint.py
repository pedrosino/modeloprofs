...
# auxiliares
desvios = [solver.IntVar(0, solver.infinity(), f"zd_{m_unidades[u][0]}") for u in range(n_unidades)]
medias = [solver.IntVar(0, solver.infinity(), f"zm_{m_unidades[u][0]}") for u in range(n_unidades)]
media_geral = solver.IntVar(0, solver.infinity(), "zmg")
...
if(modo == 'ch'):
    ...
    m = round((ch_min - ch_max) / (c - d) * fator)
    print(f"Geral, c = {c}, d = {d}, m = {m}")
    
    #model += media_geral == m*lpSum(saida[:n_unidades]) + ch_min + ch_max, "Media geral"
    solver.Add(media_geral == m*sum(saida) + ch_min*fator + ch_max*fator, "Media geral")
    for u in range(n_unidades):        
        ...
        # calculo da media
        solver.Add(medias[u] == m*sum(outra[u]) + ch_min*fator + ch_max*fator, f"{m_unidades[u][0]}_ch_media")
        # desvio
        if(modulo):
            ...
            #solver.Add(desvios[u] >= m*sum(outra[u]) + ch_min*fator + ch_max*fator - media_geral, f"{m_unidades[u][0]}_up")
            #solver.Add(desvios[u] >= -1*(m*sum(outra[u]) + ch_min*fator + ch_max*fator - media_geral), f"{m_unidades[u][0]}_low")

# Imprime os resultados no arquivo txt
original_stdout = sys.stdout
#filename = f"output{datetime.now().strftime('%H-%M-%S')}.txt"
filename = f"CHORint-{modo}_n{n_unidades}-Ma{maxima}-Mi{minima}-Peq{peq}-Tot{n_min_total if min_total else min_total}-{n_max_total if max_total else max_total}-Dif{dif_tam if dif else dif}-40h{quarenta}-20h{vinte}--{datetime.now().strftime('%H-%M-%S')}.txt"
...
df.to_excel(f'CHORint{modo}-Peq{peq}-test.xlsx', sheet_name='Resultados', index=False)
